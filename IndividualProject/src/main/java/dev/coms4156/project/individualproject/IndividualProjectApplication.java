...
public class IndividualProjectApplication implements CommandLineRunner {
	...
	public void run(String[] args) {
		...
		if (args.length == 0) {
			systemDatabase = new MyFileDatabase(0, "./data.txt");
		} else {
			systemDatabase = new MyFileDatabase(1, "./data.txt");
			resetDataFile();
		}
	}
	...
	public void resetDataFile() {
		...
		systemDatabase.setMapping(mapping);
	}
	...
	public void onTermination() {
		...
		if (saveData) {
			systemDatabase.saveContentsToFile();
		}
	}


	//Database Instance
	public static MyFileDatabase systemDatabase;

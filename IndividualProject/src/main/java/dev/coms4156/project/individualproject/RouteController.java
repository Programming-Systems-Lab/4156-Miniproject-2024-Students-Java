...
public class RouteController {
  ...
  public ResponseEntity<?> retrieveDepartment(@RequestParam String deptCode) {
    try {
      ...
      String deptKey = deptCode.toUpperCase(Locale.ROOT);
      if (departmentMapping.containsKey(deptKey)) {
        return new ResponseEntity<>(departmentMapping.get(deptKey).toString(), HttpStatus.OK);
      } else {
  ...
  public ResponseEntity<?> retrieveCourse(@RequestParam String deptCode,
      @RequestParam int courseCode) {
    try {
      ResponseEntity<?> departmentResponse = retrieveDepartment(deptCode);
      ...
      Map<String, Department> departmentMapping =
          IndividualProjectApplication.myFileDatabase.getDepartmentMapping();
      Department department = departmentMapping.get(
          deptCode.toUpperCase(Locale.ROOT));  // Assuming department keys are case insensitive
      Map<String, Course> coursesMapping = department.getCourseSelection();

      String courseKey = Integer.toString(courseCode);
      if (!coursesMapping.containsKey(courseKey)) {
        return new ResponseEntity<>("Course Not Found", HttpStatus.NOT_FOUND);
      }

      Course course = coursesMapping.get(courseKey);
   ...
   *         department and an HTTP 200 response or,
  ...
  public ResponseEntity<?> findCourseLocation(@RequestParam("deptCode")
        String deptCode, @RequestParam("courseCode") int courseCode) {
    try {
      boolean doesCourseExists;
      doesCourseExists = retrieveCourse(deptCode, courseCode).getStatusCode() == HttpStatus.OK;
   ...
   *         and an HTTP 200 response or,
   *         an appropriate message indicating the proper response.
   */
  @GetMapping(value = "/findCourseTime", produces = MediaType.APPLICATION_JSON_VALUE)
  public ResponseEntity<?> findCourseTime(@RequestParam("deptCode")
        String deptCode, @RequestParam("courseCode") int courseCode) {
    try {
      boolean doesCourseExists;
      doesCourseExists = retrieveCourse(deptCode, courseCode).getStatusCode() == HttpStatus.OK;
  ...
  public ResponseEntity<?> dropStudent(@RequestParam("deptCode")
        String deptCode, @RequestParam("courseCode") int courseCode) {
    try {
      boolean doesCourseExists;
      doesCourseExists = retrieveCourse(deptCode, courseCode).getStatusCode() == HttpStatus.OK;
  ...
  public ResponseEntity<?> setEnrollmentCount(@RequestParam("deptCode")
        String deptCode, @RequestParam("courseCode")
        int courseCode, @RequestParam("count") int count) {
    try {
      boolean doesCourseExists;
      doesCourseExists = retrieveCourse(deptCode, courseCode).getStatusCode() == HttpStatus.OK;
  ...
  public ResponseEntity<?> changeCourseTime(@RequestParam("deptCode")
        String deptCode, @RequestParam("courseCode")
        int courseCode, @RequestParam("time") String time) {
    try {
      boolean doesCourseExists;
      doesCourseExists = retrieveCourse(deptCode, courseCode).getStatusCode() == HttpStatus.OK;
  ...
  public ResponseEntity<?> changeCourseTeacher(@RequestParam("deptCode")
        String deptCode, @RequestParam("courseCode")
        int courseCode, @RequestParam("teacher") String teacher) {
    try {
      boolean doesCourseExists;
      doesCourseExists = retrieveCourse(deptCode, courseCode).getStatusCode() == HttpStatus.OK;
  ...
  public ResponseEntity<?> changeCourseLocation(@RequestParam("deptCode")
        String deptCode, @RequestParam("courseCode")
        int courseCode, @RequestParam("location") String location) {
    try {
      boolean doesCourseExists;
      doesCourseExists = retrieveCourse(deptCode, courseCode).getStatusCode() == HttpStatus.OK;

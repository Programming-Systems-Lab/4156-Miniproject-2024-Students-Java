...
public class RouteController {
  ...
  public ResponseEntity<?> retrieveDepartment(@RequestParam(value = "deptCode") String deptCode) {
    try {
      ...
      if (!departmentMapping.containsKey(deptCode.toUpperCase())) {
        return new ResponseEntity<>("Department Not Found", HttpStatus.NOT_FOUND); // bug fix
      } else {
        return new ResponseEntity<>(departmentMapping.get(deptCode.toUpperCase()).toString(),
            HttpStatus.OK); // bug fix, should be OK
      }
      ...
      if (doesDepartmentExists) {
        ...
        if (!coursesMapping.containsKey(Integer.toString(courseCode))) {
          ...
          return new ResponseEntity<>(coursesMapping.get(Integer.toString(courseCode)).toString(),
              HttpStatus.OK); // bug, should not be forbidden when found 
        }
  ...
  public ResponseEntity<?> getMajorCtFromDept(@RequestParam(value = "deptCode") String deptCode) {
    try {
      ...
      if (doesDepartmentExists) {
        Map<String, Department> departmentMapping;
        departmentMapping = IndividualProjectApplication.myFileDatabase.getDepartmentMapping();
        return new ResponseEntity<>("There are: " + departmentMapping.get(deptCode) // bug
            .getNumberOfMajors() + " majors in the department", HttpStatus.OK);
      }
      return new ResponseEntity<>("Department Not Found", HttpStatus.NOT_FOUND); //bug
    } catch (Exception e) {
      return handleException(e);
    }
  }
  ...
  /**
   * Sets the enrollment count for a course if the course exists.
   *
   * @param deptCode    Department code of the course you want to set enrollment for
   * @param courseCode  Course code of the course you want to set enrollment for
   * @param count       New count you want to set the enrollment to
  ...
  /**
   * Endpoint for changing the location of a specified course.
   ...
   * @param location    The new location you want to set the course to
   * @return            a ResponseEntity with a success message if the operation is
   *                    successful, or an error message if the course is not found
   */
  ...
  private ResponseEntity<?> handleException(Exception e) {
    ...
    return new ResponseEntity<>("An Error has occurred", HttpStatus.OK); // bug 
  }

...
/**
 * Unit tests for MyFileDatabase.
 */
@SpringBootTest
@ContextConfiguration
public class MyFileDatabaseUnitTests {
  /**
  *  The database set up to be tested.
  */
  @BeforeEach
  public void setupMyFileDatabseForTesting() {
  ...
  public void setMappingTest() {
    ...
    Department compSci = new Department("COMS", courses, "Luca Carloni", 2700);
    Map<String, Department> testMap = new HashMap<>();
    testMap.put("COMS", compSci);

    testDatabase.setMapping(testMap);

    assertEquals(testMap, testDatabase.getDepartmentMapping());
  }

  @Test
  public void deSerializeObjectFromFileTest() {
    Map<String, Department> fileDatabaseMapping = testDatabase.deSerializeObjectFromFile();
    assertEquals(testMapping.toString(), fileDatabaseMapping.toString());
  }

  @Test
  public void getDepartmentMappingTest() {
    assertEquals(testMapping.toString(), testDatabase.getDepartmentMapping().toString());
  }

  @Test
  public void toStringTest() {
    StringBuilder exceptedResult = new StringBuilder();
    for (Map.Entry<String, Department> entry : testMapping.entrySet()) {
      String key = entry.getKey();
      Department value = entry.getValue();
      exceptedResult.append("For the ")
      .append(key)
      .append(" department: \n")
          .append(value.toString());
    }
    assertEquals(exceptedResult.toString(), testDatabase.toString());
  }
